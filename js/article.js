...
async function initArticlePage() {
  ...
  await loadArticles();
  const rawArticle = articlesData.articles.find(a => a.slug === slug);
  if (!rawArticle) { window.location.href = getBasePath() + 'index.html'; return; }

  const article = typeof localizeArticle === 'function' ? localizeArticle(rawArticle) : rawArticle;
  const base = getBasePath();
  const content = getArticleContent(slug, rawArticle);

  const seoTitle = (typeof getLang === 'function' && getLang() === 'en' && rawArticle.seoTitleEn)
    ? rawArticle.seoTitleEn
    : (article.seoTitle || article.title);
  const seoDescription = (typeof getLang === 'function' && getLang() === 'en' && rawArticle.seoDescriptionEn)
    ? rawArticle.seoDescriptionEn
  ...
  const diffLabels = typeof getLang === 'function' && getLang() === 'en'
    ? { beginner: 'Beginner', intermediate: 'Intermediate', advanced: 'Advanced' }
    : { beginner: 'Principiante', intermediate: 'Intermedio', advanced: 'Avanzato' };
  const diffLabel = diffLabels[article.difficulty] || diffLabels.beginner;
  ...
  if (headerEl) {
    ...
    headerEl.innerHTML = `
      ...
      <div class="article-meta-bar">
        ...
        <span>📖 ${article.readTime} ${typeof t === 'function' ? t('pages.article.readMin') : 'min di lettura'}</span>
        <span>📅 ${typeof t === 'function' ? t('pages.article.updated') : 'Aggiornato'} ${article.date}</span>
      </div>
  ...
  const tocLabel = typeof t === 'function' ? t('pages.article.toc') : 'Indice';
  if (tocEl) tocEl.innerHTML = `<h4>${tocLabel}</h4><ul class="toc-list">${tocItems}</ul>`;
  ...
  if (bodyEl) {
    ...
    if (content.faq?.length) {
      const faqLabel = typeof t === 'function' ? t('pages.article.faq') : 'Domande frequenti';
      html += `<h2 id="faq">${faqLabel}</h2>`;
...
window.addEventListener('langchange', initArticlePage);