...
function renderHeaderSocialLink(id) {
  ...
  if (!s) return '';
  const iconHtml = socialIcon(s.id, 20) || s.name[0];
...
function renderMobileSocialLink(id) {
  const s = getSocial(id);
  if (!s) return '';
  return `<a href="${s.url}" class="mobile-nav-link" target="_blank" rel="noopener noreferrer">${socialIcon(s.id, 18) || ''} ${s.name}</a>`;
}

function renderHeaderSocialBar() {
  const links = ['instagram', 'facebook', 'x', 'linkedin'].map(renderHeaderSocialLink).join('');
  if (!links) return '';
  return `
        ...
        <div class="container header-top-inner">
          ...
          <div class="header-actions">
            <button class="search-toggle" id="open-search" aria-label="${t('nav.search')}">
              ${iconImg('search', { size: 18, className: 'site-icon site-icon--sm' })}
            </button>
            ...
            <button class="menu-toggle" id="menu-toggle" aria-label="Menu" aria-expanded="false">
              ${iconImg('menu', { size: 20, className: 'site-icon' })}
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="search-bar" id="search-bar" hidden>
        <div class="container">
          <div class="search-bar-inner">
            ${iconImg('search', { size: 18, className: 'site-icon site-icon--sm search-bar-icon' })}
            ...
            <button class="search-close" id="search-close" aria-label="${t('nav.closeSearch')}">${iconImg('close', { size: 18, className: 'site-icon site-icon--sm' })}</button>
          </div>
      ...
      <div class="mobile-nav-panel">
        <div class="mobile-nav-header">
          ...
          <div class="mobile-nav-header-actions">
            ${renderLangSwitcher()}
            <button id="mobile-nav-close" aria-label="${t('nav.close')}">${iconImg('close', { size: 18, className: 'site-icon site-icon--sm' })}</button>
          </div>
        </div>
        ...
        <nav class="mobile-nav-links">
          ...
          ${['x', 'facebook', 'instagram', 'linkedin'].map(renderMobileSocialLink).join('')}
...
function renderSocialLinks() {
  return (SITE_CONFIG.social || []).map(s =>
    `<a href="${s.url}" class="social-link social-link--${s.id}" aria-label="${s.name}" target="_blank" rel="noopener noreferrer" style="--social-color:${s.color}">${socialIcon(s.id, 22) || s.name[0]}</a>`
  ).join('');
}
...
function renderSocialCard(s) {
  ...
  return `
    <a href="${s.url}" class="social-card fade-in" target="_blank" rel="noopener noreferrer" style="--social-color:${s.color}">
      <div class="social-card-icon">${socialIcon(s.id, 26) || ''}</div>
...
function injectLayout() {
  document.getElementById('site-social-fallback')?.remove();
